<script>
	...
	import { cartesianFromPolar, xFromPolar, yFromPolar } from './utils.js';
	import { addYear, subtractYear, addMonth } from './timeUtils.js';
	...
	const findMonthDate = (date) => {
		return monthsPrevYear.filter((item) => item.getMonth() == date.getMonth())[0];
	};
...
<g transform="translate({$width / 2}, {$height / 2})">
	{#each monthProps as month}
		...
		<text>
			<textPath href="#{month.i}" startOffset="50%" text-anchor="middle" fill="currentColor"
				>{month.name}</textPath
			>
		</text>
	{/each}
